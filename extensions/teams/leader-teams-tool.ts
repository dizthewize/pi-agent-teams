...
export function registerTeamsTool(opts: {
	...
	pi.registerTool({
		...
		label: "Teams",
		description: [
			"Spawn parallel teammate agents and delegate work. Each teammate is an independent Pi process that autonomously executes assigned tasks and reports structured results back via mailbox.",
			"",
			"Best practices for task descriptions:",
			"- Be specific about deliverables (file paths, function signatures, expected behavior)",
			"- Include acceptance criteria (what 'done' looks like)",
			"- Mention constraints (tech stack, existing patterns to follow, files NOT to touch)",
			"- Reference related work (other tasks, PRs, existing code)",
			"",
			"Teammate results are structured: implementation details, files changed, verification steps, blockers resolved, and follow-up work. Review these before deciding next steps.",
			"",
		...
		promptSnippet: "Spawn teammates, delegate specific tasks with clear acceptance criteria, inspect progress and results, message workers, and manage team lifecycle.",
		promptGuidelines: [
			"Use this tool when the user wants parallel agent work, worker coordination, or team lifecycle/task management.",
			"Always include clear acceptance criteria in task descriptions so teammates know what 'done' means.",
			"Review teammate results for completeness before marking tasks complete or spawning follow-up work.",
			"Prefer member_status before interrupting or reassigning active teammates when the current state is unclear.",
			"When a task is large, split it into smaller tasks with dependencies (task_dep_add) rather than one vague description.",
		],
